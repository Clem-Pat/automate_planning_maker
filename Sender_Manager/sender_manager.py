...
class Message_sender:
    ...
    def send_email(self):
        ...
        msg['To'] = "clement.patrizio@ensta-bretagne.org"
        msg['Subject'] = "Coucou"

        msg.attach(MIMEText("Salut de clément", 'plain'))
        ...
        try:
            ...
            text = msg.as_string()
            server.sendmail(self.username, "clement.patrizio@ensta-bretagne.org", text)
            server.quit()
...
if __name__ == '__main__':
    ...
    msg = MIMEMultipart()
    msg['From'] = 'foyz-planning@gmx.com'
    msg['To'] = "clement.patrizio@ensta-bretagne.org"
    msg['Subject'] = "Coucou"
    ...
    try:
        server = smtplib.SMTP('mail.gmx.com', 587)  # Replace with your SMTP server and port
        server.starttls()
        server.login('foyz-planning@gmx.com', 'foyzplanning29')
        text = msg.as_string()
        server.sendmail('foyz-planning@gmx.com', "clement.patrizio@ensta-bretagne.org", text)
        server.quit()
        print("Email sent successfully!")
    except Exception as e:
        print(f"Failed to send email: {e}")